 ...
 * Full-screen modal for reviewing forensic report before submission
 */
class ReportSummaryModal {
    constructor(reportGenerator) {
        this.reportGenerator = reportGenerator;
        this.id = `report-summary-${reportGenerator.id}`;
        this.element = null;
    ...
    create() {
        ...
        this.element.className = 'fixed inset-0 bg-black/90 hidden z-[10000] flex justify-center items-center p-4';
        this.element.style.cssText = 'position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;';
            ...
            <div class="bg-gray-900 border border-gray-600 rounded-lg shadow-2xl w-full max-w-6xl max-h-[95vh] overflow-y-auto">
                <div class="bg-gradient-to-r from-orange-700 to-red-600 p-6 border-b border-gray-600">
                    <div class="flex justify-between items-center">
                        ...
                        <button id="close-report-modal" class="text-orange-200 hover:text-white text-3xl font-bold">&times;</button>
                    </div>
                </div>
                <div id="final-report-content" class="p-6 text-white">
                ...
                <div class="bg-gray-800 px-6 py-4 border-t border-gray-600 flex justify-between items-center">
                    <button id="back-to-editing" class="bg-gray-600 hover:bg-gray-700 text-white px-6 py-3 rounded font-semibold transition-colors">
                        <i class="bi bi-arrow-left mr-2"></i>
                        Back to Report Editor
                    </button>
                    <button id="submit-investigation" class="bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded font-semibold transition-colors">
    ...
    bindEvents() {
        const closeBtn = this.element.querySelector('#close-report-modal');
        const backBtn = this.element.querySelector('#back-to-editing');
        const submitBtn = this.element.querySelector('#submit-investigation');

        closeBtn?.addEventListener('click', () => this.hide());
        backBtn?.addEventListener('click', () => this.hide());
        submitBtn?.addEventListener('click', () => this.reportGenerator.submitInvestigation());
    }
    ...
    show() {
        ...
        const content = this.element.querySelector('#final-report-content');
        if (content) {
        ...
        console.log('[ReportSummaryModal] Report summary modal displayed');
    }
    ...
    hide() {
        ...
        console.log('[ReportSummaryModal] Report summary modal hidden');
    }
                ...
                <div class="bg-gray-800 p-6 rounded-lg border border-gray-600">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                        <div class="bg-gray-700 p-4 rounded">
                            <div class="text-2xl font-bold text-white">${this.reportGenerator.investigationScore}</div>
                            <div class="text-sm text-gray-400">Investigation Score</div>
                        </div>
                        <div class="bg-gray-700 p-4 rounded">
                            <div class="text-2xl font-bold text-white">${Array.from(this.reportGenerator.droppedEvidence.values()).reduce((total, list) => total + list.length, 0)}</div>
                            <div class="text-sm text-gray-400">Evidence Items</div>
                        </div>
                        <div class="bg-gray-700 p-4 rounded">
                            <div class="text-2xl font-bold text-white">${this.reportGenerator.droppedEvidence.size}</div>
                            <div class="text-sm text-gray-400">Report Sections</div>
                        </div>
                        <div class="bg-gray-700 p-4 rounded">
                            <div class="text-2xl font-bold text-white">${Math.round((this.reportGenerator.investigationScore / 500) * 100)}%</div>
    ...
    generateSectionsSummary() {
        if (!this.reportGenerator.reportTemplate?.sections) return '<div class="text-gray-400">No report sections available</div>';
        
        return this.reportGenerator.reportTemplate.sections.map(section => {
            const sectionEvidence = this.reportGenerator.droppedEvidence.get(section.title) || [];
    ...
    generateEvidenceSummary() {
        const allEvidence = Array.from(this.reportGenerator.droppedEvidence.values()).flat();
            ...
            return `
                <div class="bg-gray-700 p-4 rounded-lg border border-gray-600">
                    <div class="flex items-center space-x-3 mb-2">
                        <div class="text-xl">${this.reportGenerator.getEvidenceIcon(evidence.type)}</div>
                        <div class="flex-1">
                            <div class="font-semibold text-white text-sm">${evidence.name}</div>
                            <div class="text-xs text-gray-400">${this.reportGenerator.formatEvidenceType(evidence.type)}</div>
                        </div>
        ...
        const checklist = [
            {
                ...
                status: this.reportGenerator.reportTemplate?.sections?.every(section => 
                    !section.required || (this.reportGenerator.droppedEvidence.get(section.title) || []).length > 0
                ) || false
            },
            {
                item: 'Evidence integrity verified',
                status: Array.from(this.reportGenerator.droppedEvidence.values()).flat().length > 0
            },
            {
                item: 'Minimum investigation score achieved',
                status: this.reportGenerator.investigationScore >= 300
            },
            {
            ...
            },
            {
                ...
                status: this.reportGenerator.investigationScore >= 400
            }
        ];
...
export class ReportGeneratorApp extends ForensicAppBase {
    ...
    async initialize() {
        ...
        this.initializeReportSummaryModal();
    }

    initializeReportSummaryModal() {
        this.reportSummaryModal = new ReportSummaryModal(this);
    }
    ...
    generateFinalReport() {
        ...
        if (this.reportSummaryModal) {
            this.reportSummaryModal.show();
    ...
    cleanup() {
        if (this.reportSummaryModal) {
            this.reportSummaryModal.destroy();
            this.reportSummaryModal = null;
        }
        super.cleanup?.();
    }
